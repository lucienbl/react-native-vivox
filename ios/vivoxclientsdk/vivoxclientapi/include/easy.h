...
public:
    ...
    virtual int Connect(const char *serverName);
    ...
    int CreateCaptureDevice(vxa_apcd *capture_device, int *apcd_id);
    int DestroyCaptureDevice(int apcd_id);
    int CreateRenderDevice(vxa_aprd *render_device, int *aprd_id);
    int DestroyRenderDevice(int aprd_id);
