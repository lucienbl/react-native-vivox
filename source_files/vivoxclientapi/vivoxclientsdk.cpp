 ...
 * SOFTWARE.
 */
#include "include/vivoxclientsdk.h"
#include "VxcErrors.h"
#include "Vxc.h"
...
#include "include/memallocators.h"

#if defined(__APPLE__) || defined(NN_NINTENDO_SDK)
#include <pthread.h>
#include <sys/time.h>
#endif
...
{
#if defined(WIN32) || defined(_XBOX) || defined(_XBOX_ONE)
...
public:
    ...
    VCSStatus StartPlayFileIntoChannels(const char *filename)
    {
        ...
        if (nn::fs::OpenFile(&fhandle, filename, nn::fs::OpenMode::OpenMode_Read).IsFailure()) {
...
#ifdef _DEBUG
#ifdef __ANDROID__
            config.initial_log_level = log_trace;
#else // if !__ANDROID__
...
#ifdef _XBOX
                    // temporary - get us to 16 khz
                    req->default_codec = VIVOX_V_V2_AUDIO_DATA_MONO_SIREN7_16000_EXPANDED;
#endif
...
public:
    ...
    {
        return vxa_apcd_create(capture_device, apcd_id);
    }

    VCSStatus DestroyCaptureDevice(int apcd_id)
    {
        return vxa_apcd_destroy(apcd_id);
    }

    VCSStatus CreateRenderDevice(vxa_aprd *render_device, int *aprd_id)
    {
        return vxa_aprd_create(render_device, aprd_id);
    }

    VCSStatus DestroyRenderDevice(int aprd_id)
    {
        return vxa_aprd_destroy(aprd_id);
    }
