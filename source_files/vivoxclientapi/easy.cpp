...
// Connect to Server
int EasyApp::Connect(const char *serverName)
{
    Uri server(serverName);
    LOG_INFO("%s: Connecting to %s\n", NowString().c_str(), server.ToString());

    VCSStatus status = m_sdk.Connect(server);
    if (status != 0) {
...
int EasyApp::CreateCaptureDevice(vxa_apcd *capture_device, int *apcd_id)
{
    return m_sdk.CreateCaptureDevice(capture_device, apcd_id);
}

int EasyApp::DestroyCaptureDevice(int apcd_id)
{
    return m_sdk.DestroyCaptureDevice(apcd_id);
}

int EasyApp::CreateRenderDevice(vxa_aprd *render_device, int *aprd_id)
{
    return m_sdk.CreateRenderDevice(render_device, aprd_id);
}

int EasyApp::DestroyRenderDevice(int aprd_id)
{
