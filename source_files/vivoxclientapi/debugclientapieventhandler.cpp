...
* SOFTWARE.
*/
#include "include/debugclientapieventhandler.h"
...
{
#if (defined(WIN32) || defined(_XBOX) || defined(_UAP))
    OutputDebugStringA(s);
    OutputDebugStringA("\r\n");
#elif defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_INFO, "VCAPI", "%s", s);
#else
    fprintf(stdout, "%s", s);
#endif
}
...
void DebugClientApiEventHandler::onLogStatementEmitted(LogLevel level, long long nativeMillisecondsSinceEpoch, long threadId, const char *logMessage)
{
    ...
    sprintf_s(buf, sizeof(buf), "%02d:%02d:%02d.%03ld", t->tm_hour, t->tm_min, t->tm_sec, (long)tv.tv_usec / 1000);
#endif
    std::stringstream ss;
