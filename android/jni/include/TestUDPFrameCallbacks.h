#pragma once
#if !defined(__ORBIS__) && !defined(_XBOX) && !defined(_XBOX_ONE) && !TARGET_OS_IPHONE && !defined(__android__) && !defined(VXP_LINUX)
...
# ifdef __ANDROID__
static long read_long_from_file(const char *fname)
{
    long result = 0;
        ...
        )
{
    ...
    (void)frame_type;
    (void)payload_data;
    (void)payload_data_len;

    static long hdrSize = -1;
    static long trlSize = 0;
...
# ifdef __ANDROID__
        hdrSize = read_long_from_file("/sdcard/tmp/vx_test_udp_header");
        trlSize = read_long_from_file("/sdcard/tmp/vx_test_udp_trailer");
# else
        ...
        if (pVX_TEST_UDP_HEADER) {
            hdrSize = strtol(pVX_TEST_UDP_HEADER, NULL, 10);
        }

        if (pVX_TEST_UDP_TRAILER) {
            trlSize = strtol(pVX_TEST_UDP_TRAILER, NULL, 10);
        }
# endif
    }
     ...
     */
    tmp = hdr;
    for (long i = 0; i < hdrSize; i++, tmp++) {
        *tmp = i % 256;
    }
    tmp = trl;
    for (long i = 0; i < trlSize; i++, tmp++) {
        *tmp = 255 - (i % 256);
    }
    ...
    *header_len_out = (int)hdrSize;
    *trailer_len_out = (int)trlSize;
}
